...
from config import set_github_credentials
from git_operations import (
    ...
    list_directory_contents, read_file_contents, add_file_with_content,add_multiple_files
)
from github_api import create_github_repo
from config import set_github_credentials
...
# Prompt for GitHub credentials if not already set
...
# In main.py, modify the beginning to properly handle credentials

# Remove or comment out these lines at the start
...
# Add your existing CLI commands here...
...
@click.option("--checkout", is_flag=True, help="Checkout the new branch after creation")
def branch(repo_path, branch_name, checkout):
    """Create a new branch."""
    try:
        create_branch(repo_path, branch_name)
        logger.info(f"Branch '{branch_name}' created in {repo_path}")
        
        # Checkout is already handled in create_branch
    except Exception as e:
...
@click.option("--type", help="Specify project type (auto-detect if not specified)")
@click.option("--from-github", is_flag=True, help="Download template from GitHub")
def generate_gitignore_cmd(repo_path, type, from_github):
    """Generate a .gitignore file for the repository."""
    try:
        if from_github:
            download_github_gitignore(repo_path, type)
        else:
            generate_gitignore(repo_path)
...
@click.option('--input-file', '-i', type=click.Path(exists=True), help="Text file containing file paths and contents (one per line, format 'path:content')")
def add_files(repo_path, files, input_file):
    ...
    try:
        ...
        logger.info(f"Adding {len(files_dict)} files to repository at {repo_path}")
        result = add_multiple_files(repo_path, files_dict)
